...
void MultiplyVectors(int array_size, float *host_array_x, float *host_array_y, float *host_array_sum, int block_size) {
  ...
  KernelMul<<<blocks_count, block_size>>>(array_size, device_array_x, device_array_y, device_array_sum);
...
int main() {
  FILE *file = fopen("../time_measurements/vectors_multiply", "w");
  for (int array_size = 1 << 11; array_size < 1 << 28; array_size *= 8) {
    for (int block_size = 4; block_size <= 1 << 10; block_size *= 4) {
      ...
      MultiplyVectors(array_size, host_array_x, host_array_y, host_array_sum, block_size);
