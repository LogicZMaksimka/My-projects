class ImageFilter{
  ...
  PImage getDarkestColor(float darkCoefficient){
    float minBrightness = 256;
    for(int x = 0; x < img.width;++x){
      for(int y = 0; y < img.height;++y){
        int imgLoc = x + y*img.width;
        float curBrightness = brightness(img.pixels[imgLoc]);
    ...
    for(int x = 0; x < img.width;++x){
      for(int y = 0; y < img.height;++y){
        ...
        if(minBrightness + darkCoefficient >= curBrightness){
          transformedImage.pixels[imgLoc] = color(curBrightness - minBrightness);
        }
        else{
          transformedImage.pixels[imgLoc] = color(255);
        }
      }
    }
  ...
  void setDarkestColor(float darkCoefficient){
    img = getDarkestColor(darkCoefficient);
  }
  ...
  PImage getImageMultipliedByMatrix(float[][] transformationMatrix, int matrixSize){
  ...
  void multiplyImageByMatrix(float[][] transformationMatrix, int matrixSize){
    img = getImageMultipliedByMatrix(transformationMatrix, matrixSize);
  }
  ...
  void setImageSketch(float lowerLimit, float upperLimit){
    img = getImageSketch(lowerLimit, upperLimit);
  }
